...
function setupNavigation() {
    ...
    document.querySelectorAll('.back-button').forEach(btn => {
        btn.addEventListener('click', () => {
            // Simple history back for now, can be enhanced for specific view transitions
            ...
            if (currentView.id === 'folder-content-view') {
                ...
                // For simplicity, go back to folder content if currentFolderId is set, else main view
                if (currentFolderId) {
...
function loadSettings() {
    ...
    if (currentUser) {
        document.getElementById('user-email-settings').textContent = currentUser.email;
    }
...
function createFileCard(file, isFavorite = false) {
    ...
    const favoriteIcon = isFavorite ? '<i class="fas fa-star"></i>' : '<i class="far fa-star"></i>';
    fileActions = `
        <button class="favorite-file" data-id="${file.id}">${favoriteIcon}</button>
